...
class OmniVoice(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        # If the user explicitly passed False, and this is a local path, we allow it.
        if not local_files_only:
            if os.path.isdir(pretrained_model_name_or_path):
                # Local path - can be loaded
                pass
            else:
                # HF repo_id - force local_files_only
                logger.warning("Enforcing local_files_only=True for HuggingFace repo")
        ...
        try:
            ...
            if not train_mode:
                ...
                if os.path.isdir(pretrained_model_name_or_path):
                    ...
                    # Using local_files_only
                    # But first, check the local copy.
                    cache_dir = kwargs.get("cache_dir", None)
                    local_path = None
                    
                    if cache_dir:
                        # Searching in cache
                        potential_path = os.path.join(cache_dir, pretrained_model_name_or_path.replace("/", "--"))
                        if os.path.exists(potential_path):
                            local_path = potential_path
                    
                    if local_path and os.path.exists(local_path):
                        resolved_path = local_path
                        logger.info(f"Using cached model from: {resolved_path}")
                    else:
                        # We try to download only if explicitly allowed (local_files_only=False was passed))
                        if not local_files_only:
                            resolved_path = snapshot_download(
                                pretrained_model_name_or_path,
                                local_files_only=False,  # Enable downloading on first launch
                            )
                        else:
                            raise RuntimeError(
                                f"Model not found locally: {pretrained_model_name_or_path}. "
                                f"Please run with local_files_only=False first to download."
                            )

                # All tokenizer downloads from local_files_only
    ...
    def load_asr_model(self, model_name: str = "openai/whisper-large-v3-turbo"):
        ...
        # working directory
        portable_root = os.getcwd()
        
        # The models folder next to the BAT file
        local_models_dir = os.path.join(portable_root, "models")
        model_folder_name = model_name.replace("/", "--")
        local_model_path = os.path.join(local_models_dir, model_folder_name)
        
        logger.info("Looking for ASR model in: %s", local_model_path)
        
        # Checking if the model is locally available
        if os.path.exists(local_model_path) and os.path.isdir(local_model_path):
            if os.path.exists(os.path.join(local_model_path, "config.json")):
                logger.info("Found local ASR model!")
                model_path = local_model_path
            else:
                logger.warning("Model folder exists but seems empty. Re-downloading...")
                import shutil
        ...
        if model_path is None:
            logger.info("Downloading ASR model from HuggingFace (this may take a while)...")
        ...
        if cuda_available:
            ...
            logger.info("CUDA available! Loading ASR on GPU %d", device_index)
        else:
            device_index = -1
            asr_dtype = torch.float32
            logger.info("CUDA not available. Loading ASR on CPU")

        # Disable transformer logging to avoid seeing warnings
        import logging as py_logging
        transformers_logger = py_logging.getLogger("transformers")
        transformers_logger.setLevel(py_logging.ERROR)  # Только ошибки, без warnings
        
        # Suppress specific warnings using warnings.filterwarnings
        warnings.filterwarnings("ignore", message=".*forced_decoder_ids.*")
        warnings.filterwarnings("ignore", message=".*multilingual Whisper.*")
        warnings.filterwarnings("ignore", message=".*custom logits processor.*")
        warnings.filterwarnings("ignore", message=".*SuppressTokensLogitsProcessor.*")
        ...
        # Return the logging level (optional)
        transformers_logger.setLevel(py_logging.WARNING)
        
        model_device = next(self._asr_pipe.model.parameters()).device
        logger.info("ASR model loaded successfully! Real device: %s", model_device)
    ...
    def transcribe(
        ...
        try:
            if isinstance(audio, str):
                result = self._asr_pipe(audio)["text"].strip()
            else:
                waveform, sr = audio
                if waveform.dim() == 1:
                ...
                result = self._asr_pipe(audio_input)["text"].strip()
        finally:
            # Restoring logging
